    ...
    >
      ...
      <div className="space-y-3 p-3">
        {groups.slice(0, 8).map((group) => (
          <section
            key={group.traceId}
            className="rounded-md border border-bg-border/70 bg-bg-secondary/40"
          >
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-bg-border/70 px-3 py-2 text-xs">
              ...
              <div className="font-mono text-[11px] text-text-secondary">
                {t('settings.traces.spanCount').replace('{count}', String(group.rows.length))} ·{' '}
              ...
              {group.rows.map((row) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p
                      className={`mt-1 truncate text-[10px] ${row.event === 'error' ? 'text-accent-red' : 'text-text-secondary'}`}
                      title={row.errorText || attributesSummary(row.attributes)}
                    >
                  ...
                  <span
                    className={`w-fit rounded-full px-2 py-0.5 text-[11px] font-medium ${TRACE_EVENT_STYLE[row.event]}`}
                  >
                    {row.event}
                  </span>
