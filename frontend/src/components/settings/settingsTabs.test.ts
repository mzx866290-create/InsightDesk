...
describe('settingsTabs', () => {
  ...
  it('detects advanced tabs by definition list', () => {
    ...
    expect(isAdvancedSettingsTab('cloud_models')).toBe(false);
    expect(isAdvancedSettingsTab('assistant_presets')).toBe(false);
    expect(isAdvancedSettingsTab('sso')).toBe(true);
  ...
  it('uses wide modal width only for dense settings panels', () => {
    ...
    expect(getSettingsModalWidth('sso')).toBe('max-w-xl');
    expect(getSettingsModalWidth('cloud_models')).toBe('max-w-xl');
