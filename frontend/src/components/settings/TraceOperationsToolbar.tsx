    ...
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          {TRACE_LIMIT_OPTIONS.map((option) => (
            <option key={option} value={option}>
              {t('settings.traces.latest').replace('{count}', String(option))}
            </option>
          ))}
        </select>
        <Button
        ...
        <Button
          ...
          loading={clearing}
          disabled={!hasEvents}
          className="min-h-11 text-accent-red hover:text-accent-red"
          data-testid="settings-trace-clear"
        >
          <Trash2 size={12} />
          {t('settings.traces.clear')}
        </Button>
      </div>
    </div>
  )
