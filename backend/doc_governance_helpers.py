"""Document governance helpers for DocPipeline."""
...
COMPACT_DATE_TOKEN_PATTERN = re.compile(
    r"(?<!\d)((?:19|20)\d{2})(0[1-9]|1[0-2])([0-3]\d)(?!\d)"
)
EXPIRY_HINT_PATTERN = re.compile(
    r"(?:有效期至|截止(?:日期)?|失效(?:日期)?|废止(?:日期)?|截至)\s*[:：]?\s*"
...
def normalize_source_key(source: Any) -> str:
    normalized = str(source or "").strip().lower()
    normalized = normalized.replace("\\", "/")
    return re.sub(r"\s+", " ", normalized)


def normalize_topic_key(source: Any) -> str:
    ...
    stem = re.sub(
        r"(latest|final|old|new|archive|draft|superseded|历史|旧版|最新版|最终版)",
        " ",
        stem,
        flags=re.IGNORECASE,
    )
...
def parse_date_parts(year: str, month: str, day: str) -> int | None:
    try:
...
def extract_date_candidates(text: Any) -> list[int]:
    normalized = str(text or "")
    timestamps: list[int] = []
    for match in DATE_TOKEN_PATTERN.finditer(normalized):
        timestamp = parse_date_parts(match.group(1), match.group(2), match.group(3))
...
def extract_expiry_timestamp(text: Any) -> int | None:
    normalized = str(text or "")
    match = EXPIRY_HINT_PATTERN.search(normalized)
    if not match:
        return None
    if match.group(2) and match.group(3) and match.group(4):
        return parse_date_parts(match.group(2), match.group(3), match.group(4))
    if match.group(5) and match.group(6) and match.group(7):
        return parse_date_parts(match.group(5), match.group(6), match.group(7))
    return None


def extract_version_number(text: Any) -> float:
...
def extract_version_label(text: Any) -> str:
    match = VERSION_TOKEN_PATTERN.search(str(text or ""))
    if not match:
        return ""
...
def governance_rank(metadata: dict[str, Any]) -> tuple[int, float, int, int, str]:
...
def apply_document_governance_metadata(doc: Document) -> Document:
    ...
    source_key = str(metadata.get("kb_source_key") or normalize_source_key(source))
    topic_key = str(metadata.get("kb_topic_key") or normalize_topic_key(source))
    content_digest = str(metadata.get("kb_content_hash") or content_hash(doc.page_content))
    version_label = str(metadata.get("kb_version_label") or extract_version_label(source))
    if not version_label:
        version_label = extract_version_label(str(doc.page_content or "")[:500])
    version_number = float(metadata.get("kb_version_number") or extract_version_number(source))
    if version_number <= 0:
        version_number = extract_version_number(str(doc.page_content or "")[:500])
    ...
    effective_ts = int(float(metadata.get("kb_effective_ts") or 0) or 0)
    if effective_ts <= 0:
        source_dates = extract_date_candidates(source)
        effective_ts = max(source_dates) if source_dates else 0

    expiry_ts = int(float(metadata.get("kb_expiry_ts") or 0) or 0)
    if expiry_ts <= 0:
        expiry_ts = int(extract_expiry_timestamp(str(doc.page_content or "")[:1600]) or 0)
    is_expired = bool(metadata.get("kb_is_expired")) or (
        expiry_ts > 0 and expiry_ts < int(time.time())
    )
    ...
    metadata.update(
        {
            ...
            "kb_content_hash": content_digest,
            "kb_dedupe_signature": f"{topic_key}::{content_digest}",
...
def dedupe_index_documents(documents: list[Document]) -> list[Document]:
    unique_docs: list[Document] = []
    seen_signatures: set[str] = set()
    for doc in documents:
        signature = str((doc.metadata or {}).get("kb_dedupe_signature") or "").strip()
        if not signature:
            signature = content_hash(doc.page_content)
...
def apply_topic_version_status(documents: list[Document]) -> list[Document]:
...
def prepare_documents_for_index(documents: list[Document]) -> list[Document]:
    governed_docs = [apply_document_governance_metadata(doc) for doc in documents]
    deduped_docs = dedupe_index_documents(governed_docs)
    return apply_topic_version_status(deduped_docs)


def document_governance_boost(doc: Document) -> float:
