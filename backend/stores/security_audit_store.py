"""Security audit persistence store."""
...
_DETAIL_FIELD_PATTERN = re.compile(r"(?P<key>[A-Za-z_][A-Za-z0-9_-]*)=(?P<value>[^ ]*)")


def _detail_field(details: Any, *names: str) -> str:
    normalized_names = {str(name or "").strip().lower() for name in names}
    for match in _DETAIL_FIELD_PATTERN.finditer(str(details or "")):
...
class SQLiteSecurityAuditStore:
    ...
    def _init_db(self) -> None:
        with connect_sqlite(self.db_path) as conn:
            ...
            existing_columns = {
                str(row[1])
                for row in cursor.execute("PRAGMA table_info(security_audit_events)")
            }
            if "tenant_id" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE security_audit_events ADD COLUMN tenant_id TEXT DEFAULT ''"
                )
            if "org_id" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE security_audit_events ADD COLUMN org_id TEXT DEFAULT ''"
                )
            if "legal_hold" not in existing_columns:
                cursor.execute(
                    "ALTER TABLE security_audit_events ADD COLUMN legal_hold INTEGER NOT NULL DEFAULT 0"
                )
            cursor.execute(
                """
    ...
    def trim_to_latest(self, keep_latest: int = 0) -> int:
        ...
        with connect_sqlite(self.db_path) as conn:
            if normalized_keep <= 0:
                conn.execute("DELETE FROM security_audit_events WHERE legal_hold = 0")
            else:
                conn.execute(
                    """
