...
class SQLiteTaskStore:
    ...
    def _save_attachment_promotion_for_task(
        self, conn: Any, record: TaskRecord
    ) -> None:
    ...
    def prune(
        ...
        effective_ttl = max(
            0, self.ttl_seconds if ttl_seconds is None else int(ttl_seconds)
        )
        with connect_sqlite(self.db_path) as conn:
            now = time.time()
            conn.execute(
        ...
        *,
        ...
        if stale_rows:
            conn.executemany(
                ...
                [
                    (attachment_id, vector_store_path)
                    for attachment_id, vector_store_path in stale_rows
                ],
            )
